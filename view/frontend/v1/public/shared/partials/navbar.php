...
<?php } ?>
<header>
  <h1 class="logo"><a href="<?=$this->e($baseLinkUrl)?>"><?=$this->e($responseData->getSiteName())?></a></h1>
...
<?php } ?>
<?php if ($responseData->getSession()) { ?>
      <li>
        <label for="user-nav-toggle" class="nav-item user-nav-toggle-label">
          <?=$this->e($responseData->getUserName())?><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 256 256"><rect width="20" height="20" fill="none"></rect><polyline points="208 96 128 176 48 96" fill="none" stroke="#ffffff" stroke-linecap="round" stroke-linejoin="round" stroke-width="16"></polyline></svg>
        </label>
...
<?php } else { ?>
      <li><a class="nav-item" href="<?=$this->e($baseLinkUrl)?>/login"><?=$this->e($responseData->getLocalValue('navSignIn'))?></a></li>
      <li><a class="nav-item nav-button" href="<?=$this->e($baseLinkUrl)?>/register"><?=$this->e($responseData->getLocalValue('navSignUp'))?></a></li>
<?php } ?>
    </ul>
  </nav>
