...
LOCAL_FILEPATH="$HOME/code/db-backups/"
MYDIR="$(dirname "$(which "$0")")"

if [[ $(date -u '+%M') -ge 25 ]]
then
  FILENAME_DATE=$(date -u '+%Y-%m-%d_%Hh')20m
else
  FILENAME_DATE=$(date -u -v-1H '+%Y-%m-%d_%Hh')20m
fi
...
if ! gzip -dkf "${LOCAL_FILEPATH}${COMPRESSED_FILENAME}"
then
  rm "${LOCAL_FILEPATH}${BACKUP_FILENAME}.sql"
...
echo "Restoring database: mysql -u ${LOCAL_DATABASE_USER} -p'${LOCAL_DATABASE_PASS}' ${LOCAL_DATABASE_NAME} ${LOCAL_FILEPATH}/${BACKUP_FILENAME}.sql"

if ! mysql -u ${LOCAL_DATABASE_USER} -p"${LOCAL_DATABASE_PASS}" ${LOCAL_DATABASE_NAME} < "${LOCAL_FILEPATH}${BACKUP_FILENAME}.sql"
then
  rm "${LOCAL_FILEPATH}${BACKUP_FILENAME}.sql"
